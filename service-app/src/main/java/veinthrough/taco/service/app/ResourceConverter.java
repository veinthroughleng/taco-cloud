...
class ResourceConverter {
    ...
    OrderMix toOrder(Resource<Order> resource) {
        ...
        List<TacoMix> tacos = rest.getTacosOfOrder(orderId).getContent().stream()
                .map(this::toTaco)
                .collect(Collectors.toList());

        return new OrderMix(resource, new UserMix(rest.getUserOfOrder(orderId)), tacos);
    }
}
